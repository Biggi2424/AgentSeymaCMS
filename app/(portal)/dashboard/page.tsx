...
export default async function DashboardPage() {
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <Card
            ...
            tone="emerald"
          />
          <Card label="Agents online" value={`${online}/${agents.length}`} tone="cyan" />
          <Card label="Tickets" value={`${tickets.length} offen`} tone="amber" />
        </div>
  ...
  const [tenant, agents, tickets, deployments, catalogRequests] = await Promise.all([
    ...
    db.ticket.findMany({ where: { tenantId: session.tenantId }, orderBy: { createdAt: "desc" }, take: 6 }),
    db.deployment.findMany({ where: { tenantId: session.tenantId }, orderBy: { createdAt: "desc" }, take: 4 }),
...
function Card({ label, value, tone }: { label: string; value: string; tone: CardTone }) {
