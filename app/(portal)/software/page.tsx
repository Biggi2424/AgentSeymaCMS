  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Stat label="Pakete" value={packages.length.toString()} />
        <Stat label="Deployments aktiv" value={activeDeployments.toString()} />
        <Stat label="Deployment Results" value={totalResults.toString()} />
      </div>
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {packages.map((pkg) => (
            <div key={pkg.id} className="rounded-xl border border-zinc-800 bg-black/40 p-3 text-sm text-zinc-200">
          ...
          {packages.length === 0 && <p className="text-sm text-zinc-500">Keine Pakete hinterlegt.</p>}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
        ...
        <div className="mt-3 space-y-2">
          ...
          {deployments.map((deployment) => (
            <div
              key={deployment.id}
              className="flex flex-col gap-1 rounded-xl border border-zinc-800 bg-black/40 px-3 py-2 text-sm text-zinc-200 md:flex-row md:items-center md:justify-between"
            >
              <div>
